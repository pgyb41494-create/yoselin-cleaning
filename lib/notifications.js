...
async function notifyAdmin({ subject, message }) {
  ...
  // Send via Twilio SMS to all admin phone numbers
  const smsNumbers = (ADMIN_SMS() || '').split(',').map(s => s.trim()).filter(Boolean);
  if (smsNumbers.length > 0) {
    try {
      await fetch('/api/send-sms', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ to: smsNumbers, body: `${subject}\n\n${message}` }),
      });
    } catch (e) {
      console.warn('[notifications] SMS send failed:', e);
    }
  }
}
